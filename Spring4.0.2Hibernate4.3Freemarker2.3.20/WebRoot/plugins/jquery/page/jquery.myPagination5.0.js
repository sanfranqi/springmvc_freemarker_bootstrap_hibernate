	...
	function init(param, obj) {
		...
		function responseHandle(data) {
			...
			case "xml":
				...
				break;
			default:
				resultPageCount = $(data).find(
						":hidden[id='" + pageCountId + "']").val();
				break;
			}
			debug(opts.id);
		...
		function createPageBar() {
			...
			opts.currPage = opts.currPage > opts.pageCount ? opts.pageCount
					: opts.currPage;
			...
			if (opts.panel.first_on) {
				str = "<a href='" + links + "' title='1'>" + opts.panel.first
						+ "</a>";
			}
			...
			if (opts.panel.prev_on) {
				if (currPage == 1) {
					str += "<span class=\"disabled\" title=\""
							+ opts.panel.prev + "\">" + opts.panel.prev
							+ " </span>";
				} else {
					str += "<a href='" + links + "' title='" + (currPage - 1)
							+ "'>" + opts.panel.prev + " </a>";
				}
			}
			...
			firstPage = currPage - tempPage > 0 ? firstPage = currPage
					- tempPage : 1;
			...
			for (firstPage; firstPage < lastPage; firstPage++) {
				if (firstPage == currPage) {
					str += "<span class=\"current\" title=\"" + firstPage
							+ "\">" + firstPage + "</span>";
					;
				} else {
					str += "<a href='" + links + "' title='" + firstPage + "'>"
							+ firstPage + "</a>";
				}
			}
			...
			if (opts.panel.next_on) {
				if (currPage == pageCount) {
					str += "<span class=\"disabled\" title=\""
							+ opts.panel.next + "\">" + opts.panel.next
							+ " </span>";
				} else {
					str += "<a href='" + links + "' title='" + (currPage + 1)
							+ "'>" + opts.panel.next + " </a>";
			...
			if (opts.panel.last_on) {
				str += "<a href='" + links + "' title='" + pageCount + "'>"
						+ opts.panel.last + "</a>";
			}
